...
matrix = [[int(j) for j in i] for i in M]
...
for i in range(20):
    for j in range(17):
        num_hor = matrix[i][j]*matrix[i][j+1]*matrix[i][j+2]*matrix[i][j+3]
        if num_hor > product:
            product = num_hor
        num_ver = matrix[j][i]*matrix[j+1][i]*matrix[j+2][i]*matrix[j+3][i]
        if num_ver > product:
            product = num_ver

for i in range(17):
    for j in range(17):
        num_dig_lr = matrix[i][j]*matrix[i+1][j+1]*matrix[i+2][j+2]*matrix[i+3][j+3]
        if num_dig_lr > product:
            product = num_dig_lr

for i in range(17):
    for j in range(17):
        num_dig_rl = matrix[i][j+3]*matrix[i+1][j+2]*matrix[i+2][j+1]*matrix[i+3][j]
        if num_dig_rl > product:
            product = num_dig_rl
